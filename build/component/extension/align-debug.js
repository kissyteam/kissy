...
KISSY.add('component/extension/align', [
    ...
    function getVisibleRectForElement(element) {
        var visibleRect = {
                ...
                top: 0,
                bottom: Infinity
            }, el, scrollX, scrollY, winSize, doc = element.ownerDocument, $win = $(doc).getWindow(), body = doc.body, documentElement = doc.documentElement;    // Determine the size of the visible rect by climbing the dom accounting for
                                                                                                                                                                 // all scrollable containers.
        // Determine the size of the visible rect by climbing the dom accounting for
        // all scrollable containers.
        for (el = element; el = getOffsetParent(el);) {
            // clientWidth is zero for inline block elements in ie.
            if ((!UA.ie || el.clientWidth !== 0) && // body may have overflow set on it, yet we still get the entire
                // viewport. In some browsers, el.offsetParent may be
                // document.documentElement, so check for that too.
                (el !== body && el !== documentElement && $(el).css('overflow') !== 'visible')) {
                var pos = $(el).offset();    // add border
                // add border
    ...
    Align.prototype = {
        ...
        align: function (refNode, points, offset, overflow) {
            ...
            // 当前节点将要所处的区域
            var newElRegion = S.merge(elRegion, elFuturePos);    // 如果可视区域不能完全放置当前节点时允许调整
