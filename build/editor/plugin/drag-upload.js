...
KISSY.add("editor/plugin/drag-upload", ["editor", "event"], function(S, require) {
  ...
  S.augment(dragUpload, {pluginRenderUI:function(editor) {
    var cfg = this.config, fileInput = cfg["fileInput"] || "Filedata", sizeLimit = cfg["sizeLimit"] || Number["MAX_VALUE"], serverParams = cfg["serverParams"] || {}, serverUrl = cfg["serverUrl"] || "", suffix = cfg["suffix"] || "png,jpg,jpeg,gif", suffix_reg = new RegExp(suffix.split(/,/).join("|") + "$", "i"), inserted = {}, startMonitor = false;
