  ...
  S.extend(XiamiMusicDialog, FlashDialog, {_config:function() {
    ...
    function loadRecordsByPage(page) {
      ...
      S.use("io", function(S, IO) {
        new IO({cache:false, url:req, dataType:"jsonp", success:function(data) {
          data.page = page;
    ...
    self._xiamia_list.on("click", function(ev) {
      ...
      var t = new Node(ev.target), add = t.closest(function(node) {
        return self._xiamia_list.contains(node) && Dom.hasClass(node, prefixCls + "editor-xiami-add")
      }, undefined), paging = t.closest(function(node) {
        return self._xiamia_list.contains(node) && Dom.hasClass(node, prefixCls + "editor-xiami-page-item")
      }, undefined);
      if(add) {
    ...
    if(data.key == S.trim(self._xiami_input.val())) {
      ...
      self._xiamia_list.html(S.substitute(html, {prefixCls:prefixCls}))
    }
  }, _updateD:function() {
