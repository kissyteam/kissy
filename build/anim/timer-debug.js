...
KISSY.add('anim/timer', [
    ...
    S.extend(TimerAnim, AnimBase, {
        ...
        frame: function () {
            ...
            var currentTime = S.now(), duration = self.config.duration * 1000, remaining = Math.max(0, self.startTime + duration - currentTime), temp = remaining / duration || 0, percent = 1 - temp;
            self.defer.notify([
                self,
                percent,
                remaining
    ...
    // for test
    TimerAnim.Fx = Fx;
    S.mix(TimerAnim, AnimBase.Statics);    // bad
    // bad
    S.Anim = TimerAnim;
    return TimerAnim;
});    /*
...
KISSY.add('anim/timer/manager', [], function (S) {
    var stamp = S.stamp, win = S.Env.host,
...
KISSY.add('anim/timer/transform', [
    ...
    function getTransformInfo(transform) {
        ...
        var trim = S.trim, i = -1, l = transform.length - 1, split, prop, val, ret = defaultDecompose();    // Loop through the transform properties, parse and multiply them
