 ...
 */
    ...
    function fixType(cfg, type) {
        var s = Special[type] || {}, typeFix;    // in case overwritten by typeFix in special events
     ...
     */
            detach: function (targets, type, fn, context) {
                ...
                BaseUtils.batchForType(function (targets, singleType, fn, context) {
                    ...
                    for (i = targets.length - 1; i >= 0; i--) {
                        t = targets[i];
                        removeInternal(t, singleType, cfg);    // deep remove
                        // deep remove
     ...
     */
         ...
         */
                ...
                BaseUtils.splitAndRun(eventType, function (eventType) {
                    ...
                    BaseUtils.fillGroupsForEvent(eventType, eventData);    // mouseenter
                    // mouseenter
                    eventType = eventData.type;
                    var s = Special[eventType];
                    var originalType = eventType;    // where observers lie
                    ...
                    for (i = targets.length - 1; i >= 0; i--) {
                        target = targets[i];
                        domEventObservable = DomEventObservable.getDomEventObservable(target, originalType);    // bubbling
 ...
 */
     ...
     */
         ...
         */
            ...
            for (i = 0, len = allObservers.length; !event.isPropagationStopped() && i < len; ++i) {
                ...
                event.currentTarget = currentTarget0;    //noinspection JSUnresolvedFunction
                //noinspection JSUnresolvedFunction
                for (j = 0; !event.isImmediatePropagationStopped() && j < currentTargetObservers.length; j++) {
                    ...
                    ret = currentTargetObserver.notify(event, self);    // 和 jQuery 逻辑保持一致
     ...
     */
        fire: function (event, onlyHandlers) {
            ...
            // but we can not call event.halt()
            // because handle will check event.isPropagationStopped
            var cur = currentTarget, win = Dom.getWindow(cur), curDocument = win.document, eventPath = [], gret, ret, ontype = 'on' + eventType, eventPathIndex = 0;    // http://www.w3.org/TR/dom/#dispatching-events
            ...
            // or let event path be the empty list otherwise.
            do {
                eventPath.push(cur);    // Bubble up to document, then to window
                // Bubble up to document, then to window
                cur = cur.parentNode || cur.ownerDocument || cur === curDocument && win;
            } while (!onlyHandlers && cur && bubbles);
            cur = eventPath[eventPathIndex];    // bubble up dom tree
            // bubble up dom tree
            do {
                event.currentTarget = cur;
                domEventObservable = DomEventObservable.getDomEventObservable(cur, eventType);    // default bubble for html node
            ...
            if (!onlyHandlers && !event.isDefaultPrevented()) {
                ...
                try {
                    ...
                    if (currentTarget[eventType] && !util.isWindow(currentTarget)) {
                        // 记录当前 trigger 触发
                        DomEventObservable.triggeredEvent = eventType;    // 只触发默认事件，而不要执行绑定的用户回调
                                                                          // 同步触发
     ...
     */
        ...
        checkMemory: function () {
            ...
            if (eventDesc) {
                ...
                if (!self.hasObserver()) {
                    handle = eventDesc.handle;    // remove(el, type) or fn 已移除光
 ...
 */
    ...
    var DOCUMENT = document, TRUE = true, FALSE = false, commonProps = [
            ...
            },
            {
                ...
                props: [],
                fix: function (event, originalEvent) {
                    var deltaX, deltaY, delta, wheelDelta = originalEvent.wheelDelta, axis = originalEvent.axis, wheelDeltaY = originalEvent.wheelDeltaY, wheelDeltaX = originalEvent.wheelDeltaX, detail = originalEvent.detail;    // ie/webkit
                    // ie/webkit
                    if (wheelDelta) {
 ...
 */
    ...
    util.extend(DomEventObject, BaseEvent.Object, {
        ...
        preventDefault: function () {
            var self = this, e = self.originalEvent;    // if preventDefault exists run it on the original event
        ...
        stopPropagation: function () {
            var self = this, e = self.originalEvent;    // if stopPropagation exists run it on the original event
    ...
    util.each([
        ...
        Special[o.name] = {
            ...
            handle: function (event, observer, ce) {
                var currentTarget = event.currentTarget, relatedTarget = event.relatedTarget;    // 在自身外边就触发
