...
KISSY.add('kison/grammar', [
    ...
    var GrammarConst = {
            ...
            PRODUCTION_INDEX: 1,
            TO_INDEX: 2
        }, serializeObject = Utils.serializeObject, mix = S.mix, END_TAG = Lexer.STATIC.END_TAG, START_TAG = '$START';
    ...
    return Base.extend({
        ...
        buildNullable: function () {
            var self = this, i, rhs, n, symbol, t, production, productions, nonTerminals = self.get('nonTerminals'), cont = true;    // loop until no further changes have been made
        ...
        isNullable: function (symbol) {
            var self = this, nonTerminals = self.get('nonTerminals');    // rhs
            // rhs
        ...
        findFirst: function (symbol) {
            var self = this, firsts = {}, t, i, nonTerminals = self.get('nonTerminals');    // rhs
            // rhs
            if (symbol instanceof Array) {
        ...
        buildFirsts: function () {
            var self = this, nonTerminal, nonTerminals = self.get('nonTerminals'), cont = true, symbol, firsts;    // loop until no further changes have been made
            // loop until no further changes have been made
            while (cont) {
                cont = false;    // 传递
                                 // S -> T
                ...
                S.each(items, function (item) {
                    ...
                    S.each(productions, function (p2) {
                        if (p2.get('symbol') === dotSymbol) {
                            var newItem = new Item({ production: p2 }),
                                /*
                             2012-07-26
                             improve performance,
                             reduce item number,
                             ...
                             and dotPosition
                             */
                                itemIndex = itemSet.findItemIndex(newItem, true), findItem;
                            if (itemIndex !== -1) {
