    ...
    IO.on('start', function (e) {
        ...
        if (tmpForm) {
            ...
            if (isUpload && FormData) {
                c.files = c.files || {};
                S.mix(c.files, files);    // browser set contentType automatically for FileData
...
KISSY.add('io/iframe-transport', [
    ...
    var logger = S.getLogger('s/io');
    var UA = require('ua');
    var doc = S.Env.host.document, OK_CODE = 200, ERROR_CODE = 500, BREATH_INTERVAL = 30, iframeConverter = S.clone(IO.getConfig().converters.text);    // https://github.com/kissyteam/kissy/issues/304
