 ...
 */
    ...
    Dom._fixCloneAttributes = function (src, dest) {
        ...
        var srcValue, srcChecked;    // IE6-8 fail to clone children inside object elements that use
        ...
        if (nodeName === 'object' && !dest.childNodes.length) {
            ...
            dest.defaultValue = src.defaultValue;    // textarea will not keep value if not deep clone
        ...
        if (docElem.style[OPACITY] == null) {
            cssHooks[OPACITY] = {
                ...
                set: function (elem, val) {
                    val = parseFloat(val);
                    var style = elem[STYLE], currentStyle = elem[CURRENT_STYLE], opacity = isNaN(val) ? '' : 'alpha(' + OPACITY + '=' + val * HUNDRED + ')', filter = util.trim(currentStyle && currentStyle[FILTER] || style[FILTER] || '');    // ie  has layout
                    // ie  has layout
 ...
 */
    ...
    if (div.getElementsByTagName('*').length) {
        getElementsByTagName = function (name, context) {
            var nodes = context.getElementsByTagName(name), needsFilter = name === '*';    // <input id='length'>
            // <input id='length'>
 ...
 */
    ...
    function matrix(transform) {
        ...
        var trim = util.trim, i = -1, l = transform.length - 1, split, prop, val, ret = cssMatrixToComputableMatrix([
                ...
                0,
                0
            ]), curr;    // Loop through the transform properties, parse and multiply them
 ...
 */
    ...
    function getRangeText(elem, range) {
        if (elem.type === 'textarea') {
            var ret = range.text, testRange = range.duplicate();    // collapsed
            // collapsed
            if (testRange.compareEndPoints('StartToEnd', testRange) === 0) {
                return ret;
            }
            testRange.moveEnd('character', -1);    // consider end \r\n
            // consider end \r\n
 ...
 */
    ...
    if (IE_VERSION < 8) {
        ...
        // ie6,7 不区分 attribute 与 property
        util.mix(Dom._attrFix, propFix);    // http://fluidproject.org/blog/2008/01/09/getting-setting-and-removing-tabindex-values-with-javascript/
