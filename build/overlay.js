...
KISSY.add('overlay/dialog-tpl',function(){
 return '<div id="ks-contentbox{{id}}"         class="         {{prefixCls}}contentbox         {{getCssClassWithState "contentbox"}}         ">      <div class="{{prefixCls}}stdmod-header"          style=" {{#each headerStyle}}   {{xkey}}:{{.}};   {{/each}} "          id="ks-stdmod-header{{id}}">         {{{headerContent}}}     </div>      <div class="{{prefixCls}}stdmod-body"          style=" {{#each bodyStyle}}   {{xkey}}:{{.}};   {{/each}} "          id="ks-stdmod-body{{id}}">         {{{bodyContent}}}     </div>      <div class="{{prefixCls}}stdmod-footer"          style=" {{#each footerStyle}}   {{xkey}}:{{.}};   {{/each}} "          id="ks-stdmod-footer{{id}}">         {{{footerContent}}}     </div>  </div>  <div tabindex="0"      style="position: absolute"></div>';
});
/**
 * @ignore
 * KISSY.Dialog
