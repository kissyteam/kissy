...
KISSY.add('editor', [
    ...
    var TRUE = true, FALSE = false, NULL = null, window = S.Env.host, document = window.document, UA = S.UA, IS_IE = UA.ieMode < 11,
        // ie11 = UA.ieMode === 11,
        NodeType = Node.NodeType, $ = Node.all, HEIGHT = 'height', tryThese = Utils.tryThese, IFRAME_TPL = '<iframe' + ' class="{prefixCls}editor-iframe"' + ' frameborder="0" ' + ' title="kissy-editor" ' + ' allowTransparency="true" ' + ' {iframeSrc} ' + '>' + '</iframe>', EMPTY_CONTENT_REG = /^(?:<(p)>)?(?:(?:&nbsp;)|\s|<br[^>]*>)*(?:<\/\1>)?$/i;
    Editor.Mode = {
        SOURCE_MODE: 0,
        WYSIWYG_MODE: 1
    };
    ...
    function prepareIFrameHTML(id, customStyle, customLink, data) {
        ...
        return S.substitute(iframeContentTpl, {
            ...
            doctype: S.UA.ieMode === 8 ? '<meta http-equiv="X-UA-Compatible" content="IE=7" />' : '',
            title: '{title}',
            links: links,
 ...
 * @author yiminghe@gmail.com
 */
KISSY.add('editor/focus-manager', ['./base'], function (S, require) {
    var Editor = require('./base');
    function focus() {
        var self = this;
    ...
    S.augment(KERange, {
        ...
        createBookmark2: function (normalized) {
            ...
            if (normalized) {
                ...
                if (startContainer[0].nodeType === Dom.NodeType.ELEMENT_NODE) {
                    child = new Node(startContainer[0].childNodes[startOffset]);    // In this case, move the start information to that text
                                                                                    // node.
                ...
                if (!self.collapsed) {
                    ...
                    if (endContainer[0].nodeType === Dom.NodeType.ELEMENT_NODE) {
                        child = new Node(endContainer[0].childNodes[endOffset]);    // In this case, move the start information to that
                                                                                    // text node.
        ...
        moveToBookmark: function (bookmark) {
            ...
            if (bookmark.is2) {
                ...
                var serializable = bookmark.serializable, startNode = serializable ? S.one('#' + bookmark.startNode, doc) : bookmark.startNode, endNode = serializable ? S.one('#' + bookmark.endNode, doc) : bookmark.endNode;    // Set the range start at the bookmark start node position.
        ...
        splitBlock: function (blockTag) {
            ...
            if (startBlock && startBlock[0] === endBlock[0]) {
                if (isEndOfBlock) {
                    ...
                    // Note: bogus <br> added under <ul> or <ol> would cause
                    // lists to be incorrectly rendered.
                    if (!UA.ie && !S.inArray(startBlock.nodeName(), [
                            'ul',
                            'ol'
                        ])) {
    ...
    function mergeElements(element, isNext) {
        ...
        if (sibling && sibling[0].nodeType === NodeType.ELEMENT_NODE) {
            ...
            if (element._4eIsIdentical(sibling, undefined)) {
                ...
                var innerSibling = new Node(isNext ? element[0].lastChild : element[0].firstChild);    // Move pending nodes first into the target element.
    ...
    S.augment(KESelection, {
        ...
        createBookmarks: function (serializable, ranges) {
            ...
            for (var i = 0; i < length; i++) {
                ...
                var bookmarkStart = serializable ? S.one('#' + bookmark.startNode, doc) : bookmark.startNode, bookmarkEnd = serializable ? S.one('#' + bookmark.endNode, doc) : bookmark.endNode;    // Updating the offset values for rest of ranges which have been mangled(#3256).
...
KISSY.add('editor/enter-key', [
    ...
    function enterBlock(editor) {
        ...
        if (!isStartOfBlock && !isEndOfBlock) {
            ...
            if (nextBlock.nodeName() === 'li' && (node = nextBlock.first(Walker.invisible(true))) && S.inArray(node.nodeName(), [
                    'ul',
                    'ol'
                ])) {
        ...
        if (!OLD_IE) {
            if (nextBlock) {
                // If we have split the block, adds a temporary span at the
                // range position and scroll relatively to it.
                var tmpNode = new Node(doc.createElement('span'));    // We need some content for Safari.
...
.replace(/\xa0/g, '&nbsp;');
            ...
            editor.htmlDataProcessor = {
                ...
                toDataFormat: function (html, _dataFilter) {
                    ...
                    // Call the browser to help us fixing a possibly invalid HTML
                    // structure.
                    var div = new Node('<div>');    // Add fake character to workaround IE comments bug. (#3801)
...
KISSY.add('editor/selection-fix', [
    ...
    function fixSelectionForIEWhenDocReady(editor) {
        var doc = editor.get('document')[0], body = new Node(doc.body), html = new Node(doc.documentElement);    //ie 焦点管理不行 (ie9 也不行) ,编辑器 iframe 失去焦点，选择区域/光标位置也丢失了
        ...
        body.on('focusin', function (evt) {
            var t = new Node(evt.target);    // If there are elements with layout they fire this event but
    ...
    function monitorSelectionChange(editor) {
        ...
        editor.on('selectionChange', function (ev) {
            ...
            var path = ev.path, editorDoc = editor.get('document')[0], body = new Node(editorDoc.body), selection = ev.selection, range = selection && selection.getRanges()[0],
...
KISSY.add('editor/styles', [
 ...
 : //yiminghe note:no need!
    ...
    function getElement(style, targetDocument, element) {
        ...
        // Create the element.
        el = new Node(targetDocument.createElement(elementName));    // #6226: attributes should be copied before the new ones are applied
...
.toLowerCase();
    ...
    function removeFromElement(style, element) {
        var def = style._.definition, overrides = getOverrides(style), attributes = S.merge(def.attributes, (overrides[element.nodeName()] || overrides['*'] || {}).attributes), styles = S.merge(def.styles, (overrides[element.nodeName()] || overrides['*'] || {}).styles),
            // If the style is only about the element itself, we have to remove the element.
            removeEmpty = S.isEmptyObject(attributes) && S.isEmptyObject(styles);    // Remove definition attributes/style from the element.
        ...
        getNextParagraph: function (blockTag) {
            ...
            if (!block) {
                ...
                if ((!block || !block[0]) && !self.enforceRealBlocks && checkLimits[startBlockLimit.nodeName()] && range.checkStartOfBlock() && range.checkEndOfBlock()) {
                    ...
                    block = new Node(self.range.document.createElement(blockTag || 'p'));    // Move the contents of the temporary range to the fixed block.
