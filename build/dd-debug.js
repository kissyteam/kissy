...
KISSY.add('dd/draggable', [
    ...
    var Draggable = Base.extend({
            ...
            start: function () {
                ...
                if (node) {
                    node.on(DragGesture.DRAG_START, onDragStart, self).on(DragGesture.DRAG, onDrag, self).on(DragGesture.DRAG_END, onDragEnd, self).on(BaseGesture.START, onGestureStart, self)    // android need to prevent on touchmove
.on([
                        'dragstart',
                        'touchmove'
                    ], preventDefault);
            ...
            stop: function () {
                var self = this, node = self.getEventTargetEl();
                if (node) {
                    node.detach(DragGesture.DRAG_START, onDragStart, self).detach(DragGesture.DRAG, onDrag, self).detach(DragGesture.DRAG_END, onDragEnd, self).detach(BaseGesture.START, onGestureStart, self).detach([
                        'dragstart',
                        'touchmove'
                    ], preventDefault);
                }
