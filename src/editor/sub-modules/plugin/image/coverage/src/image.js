...
function visit2_17_2(result) {
  _$jscoverage['/image.js'].branchData['17'][2].ranCondition(result);
  return result;
}_$jscoverage['/image.js'].branchData['17'][1].init(45, 139, 'node.nodeName() === \'img\' && (!/(^|\\s+)ke_/.test(node[0].className))');
function visit1_17_1(result) {
  _$jscoverage['/image.js'].branchData['17'][1].ranCondition(result);
  return result;
...
KISSY.add(function(S, require) {
  ...
  if (visit1_17_1(visit2_17_2(node.nodeName() === 'img') && (!/(^|\s+)ke_/.test(node[0].className)))) {
    _$jscoverage['/image.js'].lineData[20]++;
    return node;
  }
