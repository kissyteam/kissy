/**
 * @fileOverview buttons or controls of toolbar
 * @author dxq613@gmail.com
 */
KISSY.add('grid/baritem',function(S,UIBase,Component,Button){
	
	var BarItemRender =  UIBase.create(Component.Render,{

		renderUI:function(){
			var el = this.get("el");
			el.addClass("ks-inline-block");
		}
	});
	/**
     * BarItem class a control used in toolbar ,for example button,select,text,input an so on
     * @name BarItem
     * @constructor
     * @extends Component.Controller
     * @memberOf Grid.Bar
     */
	var BarItem = UIBase.create(Component.Controller,{
		/* render baritem's dom
		* @protected
        * @override
		*/
		renderUI:function() {
            var el = this.get("el");
 
            if (!el.attr("id")) {
                el.attr("id", S.guid("ks-bar-item"));
            }
        },
		/*
		* bind custom event
		* @protected
        * @override
		*/
		bindUI : function(){
			var _self = this,
				listeners = _self.get('listeners');
			
			for(var name in listeners){
				if(listeners.hasOwnProperty(name) && S.isFunction(listeners[name])){
					_self.get('el').on(name,listeners[name]);
				}
			}
		}
	},{
		ATTRS:/** @lends Grid.Bar.BarItem.prototype*/
		{
			/*
			* custom listeners user can bind to barItem
			* @example 
			* listeners : {
			*	'click' : function(event){
			*		
			*	},
			*	'change' : function(){
			*
			*	}
			* }
			*/
			listeners : {
				value : {}
			},
			/**
			* Whether this component can get focus.
			* @overrided
			* @default {Bealoon} false
			*/
			focusable : {
				value : false
			}
		},
		
		
		DefaultRender : BarItemRender
	});

	Component.UIStore.setUIConstructorByCssClass("bar-item", {
        priority:Component.UIStore.PRIORITY.LEVEL1,
        ui:BarItem
    });

	/**
     * A simple class that adds button to toolbar
     * @name Button
     * @constructor
     * @extends  Grid.Bar.BarItem
     * @memberOf Grid.Bar
     */
	var ButtonBarItem = UIBase.create(BarItem,{

		initializer : function(){
			var _self = this,
				children = _self.get('children'),
				content = _self.get('content');
			children.push(new Button({
				content : content,
				disabled : _self.get('disabled')
			}));

		},
		handleKeyEventInternal:function (e) {
			if (e.keyCode == KeyCodes.ENTER &&
				e.type == "keydown" ||
				e.keyCode == KeyCodes.SPACE &&
					e.type == "keyup") {
				return this.performActionInternal(e);
			}
			// Return true for space keypress (even though the event is handled on keyup)
			// as preventDefault needs to be called up keypress to take effect in IE and
			// WebKit.
			return e.keyCode == KeyCodes.SPACE;
		},
		performActionInternal:function () {
			var self = this;
			// button 的默认行为就是触发 click
			self.fire("click");
		},
		_uiSetDisabled : function(value){
			var _self = this,
				children = _self.get('children');
			if(children[0]){
				children[0].set('disabled',value);
			}
			//_self.constructor.superclass._uiSetDisabled.call(_self,value);
		},
		_uiSetChecked: function(value){
			var _self = this,
				children = _self.get('children'),
				method = value ? 'addClass' : 'removeClass';
			if(children[0]){
				children[0].get('el')[method]('ks-button-checked');
			}
		}
		
	},{
		ATTRS:{
			checked : {
				value :false
			}
		}
	});
	
	Component.UIStore.setUIConstructorByCssClass("bar-item-button", {
        priority:Component.UIStore.PRIORITY.LEVEL2,
        ui:ButtonBarItem
    });
	/**
     * A simple class that adds a vertical separator bar between toolbar items
     * @name Separator
     * @constructor
     * @extends  Grid.Bar.BarItem
     * @memberOf Grid.Bar
     */
	var SeparatorBarItem = UIBase.create(BarItem,{
		/* render separator's dom
		* @protected
        * @override
		*/
		renderUI:function() {
            var el = this.get("el");
            el .attr("role", "separator");
        }
	});

	Component.UIStore.setUIConstructorByCssClass("bar-item-separator", {
        priority:Component.UIStore.PRIORITY.LEVEL2,
        ui:SeparatorBarItem
    });
	
	/**
     * A simple element that adds extra horizontal space between items in a toolbar
     * @name Spacer
     * @constructor
     * @extends  Grid.Bar.BarItem
     * @memberOf Grid.Bar
     */
	var SpacerBarItem = UIBase.create(BarItem,{
		
	},{
		ATTRS:/** @lends Grid.Bar.Spacer.prototype*/
		{
			/**
			* width of horizontal space between items
			* @default {Number} 2
			*/
			width : {
				value : 2
			}
		}
	});
	Component.UIStore.setUIConstructorByCssClass("bar-item-spacer", {
        priority:Component.UIStore.PRIORITY.LEVEL2,
        ui:SpacerBarItem
    });

	/**
     * A simple class that renders text directly into a toolbar.
     * @name TextItem
     * @constructor
     * @extends  Grid.Bar.BarItem
     * @memberOf Grid.Bar
     */
	var TextBarItem = UIBase.create(BarItem,{
		_uiSetText : function(text){
			var _self = this,
				el = _self.get('el');
			el.html(text);
		}
	},{
		ATTRS:/** @lends Grid.Bar.TextBarItem.prototype*/
		{
			/**
			* The text to be used as innerHTML (html tags are accepted).
			* @default {String} ""
			*/
			text : {
				value : ''
			}
		}
	});
	Component.UIStore.setUIConstructorByCssClass("bar-item-text", {
        priority:Component.UIStore.PRIORITY.LEVEL2,
        ui:TextBarItem
    });

	BarItem.types = {
		'button' : ButtonBarItem,
		'separator' : SeparatorBarItem,
		'spacer' : SpacerBarItem,
		'text'	: TextBarItem
	};
	

	return	BarItem;
},{
	requires:['uibase','component','button']
});