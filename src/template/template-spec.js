    ...
    describe('statement', function() {
        ...
        describe('each', function() {
            ...
            it('support custom value, index', function() {
                expect(T('{{#each a as}}<{{_ks_value.a}}{{/each}}').render({a: [{a: 1}, {a: 2}, {a: 3}]})).toBe('<1<2<3');
                expect(T('{{#each a as value}}<{{value.a}}{{/each}}').render({a: [{a: 1}, {a: 2}, {a: 3}]})).toBe('<1<2<3');
                expect(T('{{#each a as value index}}<{{index}}:{{value.a}}{{/each}}').render({a: [{a: 1}, {a: 2}, {a: 3}]})).toBe('<0:1<1:2<2:3');
            });
        });
