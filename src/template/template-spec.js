    ...
    describe('statement', function() { // {{{
        ...
        describe('each', function() {
            it('support each function', function() {
                expect(T('{{#each a}}<{{_ks_value.a}}{{/each}}').render({a: [{a: 1}, {a: 2}, {a: 3}]})).toEqual('<1<2<3');
                expect(T('{{#each a}}{{#if _ks_value.a > 1}}{{_ks_value.a}}{{/if}}{{/each}}').render({a: [{a: 1}, {a: 2}, {a: 3}]})).toEqual('23');
            });
            it('support custom value, index', function() {
                expect(T('{{#each a as}}<{{_ks_value.a}}{{/each}}').render({a: [{a: 1}, {a: 2}, {a: 3}]})).toEqual('<1<2<3');
                expect(T('{{#each a as value}}<{{value.a}}{{/each}}').render({a: [{a: 1}, {a: 2}, {a: 3}]})).toEqual('<1<2<3');
                expect(T('{{#each a as value index}}<{{index}}:{{value.a}}{{/each}}').render({a: [{a: 1}, {a: 2}, {a: 3}]})).toEqual('<0:1<1:2<2:3');
            });
        });
    ...
    describe('nested', function() { // {{{
        it('supports nested', function() {
            ...
            .render({users: [{
                show: false,
                name: 'Frank',
                friends: [{ name: 'jolin' }, { name: 'jolin2' }]
            }, {
                show: true,
                name: 'yyfrankyy',
                friends: [{ name: 'angela' }]
