...
KISSY.add("digital-clock", function(S, undefined) {
    ...
    S.extend(ClockNumber, S.Base, {
        ...
        zoom: function (e) {
            var z = e.newVal;
            var self = this;
            self._domNode.css(MARGIN_LEFT, C_MARGIN_LR * z + "px");
            self._domNode.css(MARGIN_RIGHT, C_MARGIN_LR * z + "px");
            ...
            self._domNode.all(ks_digitalclock_vertical).each(function (node) {
                node.css(WIDTH, VERTICAL_WIDTH * z + "px");
            });
            self._domNode.all(ks_digitalclock_bar).each(function (node) {
    ...
    function DigitalClock(cfg) {
        ...
        for (i = 0; i < 2; i++) {
            var s = new ClockNumber({zoomLimit:this.get("zoomLimit")});
            s._domNode.addClass("ks-digitalclock-seconds");
            s._domNode.appendTo(self._container[0]);
            self._ns.push(s);
        }
    ...
    S.extend(DigitalClock, S.Base, {
        zoom: function (e) {
            ...
            self._colon.one(ks_digitalclock_colon1).css(LEFT, COLON_LEFT * z + "px");
            self._colon.one(ks_digitalclock_colon2).css(LEFT, COLON_LEFT * z + "px");
            self._colon.one(ks_digitalclock_colon1).css(TOP, COLON1_TOP * z + "px");
            self._colon.one(ks_digitalclock_colon2).css(TOP, COLON2_TOP * z + "px");
