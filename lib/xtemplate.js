/**
 * @ignore
 * load tpl from file in nodejs
 * @author yiminghe@gmail.com
 */
var S = require('./seed');
var fs = require('fs');
var iconv;
try {
    iconv = require('iconv-lite');
} catch (e) {
}
var XTemplate = S.nodeRequire('xtemplate');

var cached = {};

var globalConfig = {
    extName: 'html',
    encoding: 'utf-8'
};

var packageSet, viewDir;
var Path = require('path');

function normalizeSlash(str) {
    return str.replace(/\\/g, '/');
}

function merge(config) {
    var ret = {};
    for (var i in globalConfig) {
        ret[i] = globalConfig[i];
    }
    if (config) {
        for (i in config) {
            ret[i] = config[i];
        }
    }
    return ret;
}

function renderFile(path, options, callback) {
    // console.log(options);
    if (!packageSet) {
        packageSet = 1;
        S.config('packages', {
            'views': {
                base: (viewDir = Path.dirname(options.settings.views))
            }
        });
    }
    var extName = options.settings['view engine'];
    var moduleName = normalizeSlash(path.substring(viewDir.length + 1).slice(0, -extName.length - 1));
    try {
        callback(null, module.exports.loadFromModuleName(moduleName, {
            cache: options.cache,
            extName: extName
        }).render(options));
    } catch (e) {
        callback(e);
    }
}

function XTemplateNodejs(tpl, cfg) {
    if (cfg) {
        if (cfg.cache) {
            this.cacheFile = 1;
            delete cfg.cache;
        }
        this.extName = cfg.extName;
        this.encoding = cfg.encoding;
    }
    XTemplateNodejs.superclass.constructor.apply(this, arguments);
}

function findTpl(moduleName, extName, encoding) {
    var module = new S.Loader.Module({
        name: moduleName,
        type: extName,
        runtime: S
    });

    var tpl;

    if (encoding === 'utf-8') {
        tpl = fs.readFileSync(new S.Uri(module.getPath()).getPath(), {
            encoding: 'utf-8'
        });
    } else if (iconv) {
        var buf = fs.readFileSync(new S.Uri(module.getPath()).getPath());
        tpl = iconv.decode(buf, encoding);
    } else {
        tpl = 'encoding: ' + encoding + ', npm install iconv-lite, please!';
    }
    return tpl;
}

S.extend(XTemplateNodejs, XTemplate, {
    load: function (subTplName) {
        var cacheFile = this.cacheFile;

        if (cacheFile && cached[subTplName]) {
            return cached[subTplName];
        }

        var tpl = findTpl(subTplName, this.extName, this.encoding);

        var ret = this.derive(subTplName, tpl);
        ret.extName = this.extName;
        ret.encoding = this.encoding;
        ret.cacheFile = this.cacheFile;

        if (cacheFile) {
            cached[subTplName] = ret;
        }
        return ret;
    }
});

/**
 * load xtemplate from file on nodejs
 * @singleton
 */
module.exports = {
    config: function (options) {
        S.mix(globalConfig, options);
    },

    XTemplate: XTemplate,

    __express: renderFile,

    /**
     * load xtemplate module on nodejs and return xtemplate instance
     * @param {String} moduleName xtemplate module name
     * @param config xtemplate config object
     * @param {String} [config.extName='html'] template file's extension name
     * @returns {KISSY.XTemplate} xtemplate instance
     */
    loadFromModuleName: function (moduleName, config) {
        var cache = config && config.cache;
        if (cache && cached[moduleName]) {
            return cached[moduleName];
        }
        config = merge(config);
        config.name = moduleName;
        var tpl = findTpl(moduleName, config.extName, config.encoding);
        var ret = new XTemplateNodejs(tpl, config);
        if (cache) {
            cached[moduleName] = ret;
        }
        return ret;
    }
};

