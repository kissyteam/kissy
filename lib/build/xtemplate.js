...
KISSY.add('xtemplate/compiler', [
    ...
    require('util');    // codeTemplates --------------------------- start
    // codeTemplates --------------------------- start
    ...
    var CHECK_VERSION = [
            ...
            'throw new Error("current xtemplate file(" + engine.name + ")(v{version}) ' + 'need to be recompiled using current kissy(v"+ S.version+")!");',
            '}'
        ].join('\n');
    var FUNC = [
            'function({params}){',
    ...
    var BUFFER_WRITE = 'buffer.write({value},{escape});';    // codeTemplates ---------------------------- end
    // codeTemplates ---------------------------- end
    ...
    function genTopFunction(xtplAstToJs, statements) {
        ...
        if (xtplAstToJs.isModule) {
            source.push(substitute(CHECK_VERSION, { version: S.version }));
        }
        for (var i = 0, len = statements.length; i < len; i++) {
            pushToArray(source, xtplAstToJs[statements[i].type](statements[i]).source);
        }
