 ...
 */
     ...
     */
            getHSL: function () {
                var self = this, r = self.get('r') / 255, g = self.get('g') / 255, b = self.get('b') / 255, max = Math.max(r, g, b), min = Math.min(r, g, b), delta = max - min, h, s = 0, l = 0.5 * (max + min);    // min==max means achromatic (hue is undefined)
